...
namespace Acr.UserDialogs
{
	...
	public abstract class AbstractUserDialogs : IUserDialogs
	{
		...
		public virtual Task<PromptResultMultiInput> PromptAsyncMultiInputs(string message, List<PromptInput> promptInputs, string title = null, string okText = null, string cancelText = null, string placeholder = "", CancellationToken? cancelToken = default(CancellationToken?))
		{
			return this.PromptAsyncMultiInputs(new PromptConfigMultiInput
			{
				Message = message,
				Title = title,
				CancelText = cancelText ?? PromptConfigMultiInput.DefaultCancelText,
				OkText = okText ?? PromptConfigMultiInput.DefaultOkText,
				Inputs = promptInputs,
			}, cancelToken);
		}
