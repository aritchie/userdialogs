...
namespace Acr.UserDialogs
{
	public class UserDialogsImpl : AbstractUserDialogs
	{
		...
		public override IDisposable MultiPickerPrompt(MultiPickerPromptConfig config)
		{
			var activity = this.TopActivityFunc();
			if (activity is AppCompatActivity)
				return this.ShowDialog<MultiPickerAppCompatDialogFragment, MultiPickerPromptConfig>((AppCompatActivity)activity, config);

			if (activity is FragmentActivity)
				return this.ShowDialog<MultiPickerDialogFragment, MultiPickerPromptConfig>((FragmentActivity)activity, config);

			return this.Show(activity, () => new MultiPickerBuilder().Build(activity, config));
		}
