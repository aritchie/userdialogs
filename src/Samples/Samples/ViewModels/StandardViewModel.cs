	...
	public class StandardViewModel : AbstractViewModel
	{
		...
		public StandardViewModel(IUserDialogs dialogs) : base(dialogs)
		{
			this.Commands = new List<CommandViewModel>
			{
				...
				new CommandViewModel
				{
					Text = "Prompt Multi Input",
					Command = new Command(async () =>
					{
						//define prompt input type and properties
						List<PromptInput> promptInputs = new List<PromptInput>();
						promptInputs.Add(new PromptInput {
						...
						var result = await this.Dialogs.PromptAsyncMultiInputs(new PromptConfigMultiInput
						{
							Title = "Multi Inputs",
							Inputs = promptInputs
						});
						...
						foreach (var item in result.PromptInputs) {
							sb.Append(" " + item.Placeholder + ":" + item.Text);
						}

						this.Result($"Result - {result.Ok} - {sb.ToString()}");
					})
				},
				new CommandViewModel
				{
